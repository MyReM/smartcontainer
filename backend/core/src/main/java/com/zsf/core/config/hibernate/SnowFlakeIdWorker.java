...
public class SnowFlakeIdWorker {
     ...
     * @return
     */
    public SnowFlakeIdWorker getSnowFlakeIdWorker() throws Exception {
        //加载配置文件
        InputStream is = SnowFlakeIdWorker.class.getResourceAsStream("/snowflake.properties");
        Properties properties = new Properties();
        try {
            properties.load(is);
        } catch (IOException e) {
            e.printStackTrace();
            throw new BusinessException("加载snowflake文件异常" + e.getMessage());
        }
        wordid = properties.getProperty("workId");
        dataid = properties.getProperty("dataId");
        //校验获取到参数后在进行加载
        if (StringUtils.hasLength(dataid) && StringUtils.hasLength(wordid)) {
