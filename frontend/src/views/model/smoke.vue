...
<div id="smoke">
	<div style="width: 96%;margin: auto;padding:20px 0 0 0;">
		...
		<el-table v-loading.body="listLoading" :data="Smoke" size="mini" element-loading-text="Loading"  stripe border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="warning" round style="height:25px;padding:5px 10px;fond-size:12px;margin-top:3px;" @click="updateSmoke(scope.row)">修改信息</el-button>
        </template>
      </el-table-column>
	  </el-table>
    <!-- ----------------------RFID-------------------------- -->
  ...
  <!-- 添加和修改烟类信息 -->
	<div id="au">
		<el-dialog :title="smokeDialog.title" :visible.sync="smokeDialog.visiable" @close="closeDialog">
		  <el-form :model="temp" :rules="rules" ref="ruleForm" :label-position="labelPosition" label-width="60px" v-bind:class="{ show: smokeDialog.status == 'check' }">
			  <el-form-item label="名称" prop="name">
			  	<el-input v-model="temp.name" :disabled="smokeDialog.status == 'check'" placeholder="名称"></el-input>
				</el-form-item>
        <el-form-item label="生产日期" prop="madeDate">
          <el-date-picker
            ...
            placeholder="选择日期">
			  	...
			  	<!-- <el-input v-model="temp.madeDate" :disabled="smokeDialog.status == 'check'" placeholder="生产日期"></el-input> -->
				</el-form-item>
        <el-form-item label="价格" prop="price">
			  	<el-input v-model="temp.price" type="number" :disabled="smokeDialog.status == 'check'" placeholder="价格"></el-input>
				</el-form-item>
        <el-form-item label="描述" prop="message">
			  	<el-input v-model="temp.message" :disabled="smokeDialog.status == 'check'" placeholder="描述"></el-input>
				</el-form-item>
        ...
        <el-form-item label="在库数量" prop="sum">
	      ...
	      <el-button @click="smokeDialog.visiable=false">取消</el-button>
	    	<el-button v-if="smokeDialog.status == 'create'" @click="addSmoke" type="primary">确认</el-button>
	    	<el-button v-if="smokeDialog.status == 'update'" type="primary" @click="changeSmoke">确认</el-button>
	  	</div>
		</el-dialog>
	</div>
...
<script>
import { smoke, getAllSmoke, updateSmoke } from '@/api/smoke'
...
export default {
  ...
  methods: {
    ...
    getAllSmoke() {
      ...
      getAllSmoke().then(response => {
        this.Smoke = response.data
        this.listLoading = false
      })
    },
    updateSmoke(row) {
      this.temp = Object.assign({}, row)
      this.smokeDialog.visiable = true
      this.smokeDialog.title = '修改酒类信息'
      this.smokeDialog.status = 'update'
    },
    changeSmoke() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          updateSmoke(this.temp).then(res => {
            if (res.code === 200) {
              this.getAllSmoke()
              this.smokeDialog.visiable = false
              this.$message({
                type: 'success',
                message: '修改成功!'
              })
      ...
      this.$refs['rfidForm'].validate((valid) => {
        if (valid) {
          if (this.rfidDialog.status === 'update') {
            ...
            this.commoditiesCode.commoditiesType = 2
...
<style>
  ...
  #smoke .el-form-item__label {
    float: left;
    width: 29%;
  }
  #smoke .el-form-item__content{
    display: inline-block;
  }
  #smoke #pigarea {
    width: 100%;
  }
  #smoke #pigarea .el-form-item__label {
    display: block;
    width: 100%;
    text-align: center;
  }
  #smoke #pigarea .el-form-item__content {
    display: block;
    width: 50%;
    margin: 0 0 0 25%;
  }
  #smoke #pigarea .el-form-item__content textarea{
    text-align: center;
    height: 100px;
  }
  #smoke .el-table--mini td {
    padding: 0;
    height: 35px;
    font-size: 12px;
  }
  #smoke .el-table--mini th {
    padding: 0;
    height: 40px;
    font-size: 14px;
  }
