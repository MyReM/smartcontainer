<template>
<div id="healthProducts">
	<div style="width: 96%;margin: auto;padding:20px 0 0 0;">
		
		<el-table v-loading.body="listLoading" :data="healthProducts" size="mini" element-loading-text="Loading"  stripe border fit highlight-current-row>
      ...
      <el-table-column label="过期日期" prop="overDate" align="center"></el-table-column>

      <el-table-column label="功效" prop="effect" align="center"></el-table-column>

      <el-table-column label="成分" prop="madeData" align="center"></el-table-column>
      ...
      <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="warning" round style="height:25px;padding:5px 10px;fond-size:12px;margin-top:3px;" @click="updateHealthProducts(scope.row)">修改信息</el-button>
        </template>
      </el-table-column>
	  </el-table>
    <!-- ----------------------RFID-------------------------- -->
  ...
  <!-- 添加和修改保健品类信息 -->
	<div id="au">
		<el-dialog :title="healthProductsDialog.title" :visible.sync="healthProductsDialog.visiable" @close="closeDialog">
		  <el-form :model="temp" :rules="rules" ref="ruleForm" :label-position="labelPosition" label-width="60px" v-bind:class="{ show: healthProductsDialog.status == 'check' }">
			  <el-form-item label="名称" prop="name">
			  	<el-input v-model="temp.name" :disabled="healthProductsDialog.status == 'check'" placeholder="名称"></el-input>
				</el-form-item>
        <el-form-item label="生产日期" prop="madeDate">
          <el-date-picker
            ...
            placeholder="选择日期">
			  	...
			  	<!-- <el-input v-model="temp.madeDate" :disabled="healthProductsDialog.status == 'check'" placeholder="生产日期"></el-input> -->
				</el-form-item>
        <el-form-item label="过期日期" prop="overDate">
          <el-date-picker
            v-model="temp.overDate"
            type="date"
            format="yyyy 年 MM 月 dd 日"
            value-format="yyyy-MM-dd"
            placeholder="选择日期">
        ...
        <el-form-item label="功效" prop="effect">
			  	<el-input v-model="temp.effect" :disabled="healthProductsDialog.status == 'check'" placeholder="功效"></el-input>
				</el-form-item>
        <el-form-item label="成分" prop="madeData">
			  	<el-input v-model="temp.madeData" :disabled="healthProductsDialog.status == 'check'" placeholder="成分"></el-input>
				</el-form-item>
        <el-form-item label="价格" prop="price">
			  	<el-input v-model="temp.price" type="number" :disabled="healthProductsDialog.status == 'check'" placeholder="价格"></el-input>
				</el-form-item>
        <el-form-item label="描述" prop="message">
			  	<el-input v-model="temp.message" :disabled="healthProductsDialog.status == 'check'" placeholder="描述"></el-input>
				</el-form-item>
        ...
        <el-form-item label="在库数量" prop="sum">
	      ...
	      <el-button @click="healthProductsDialog.visiable=false">取消</el-button>
	    	<el-button v-if="healthProductsDialog.status == 'create'" @click="addHealthProducts" type="primary">确认</el-button>
	    	<el-button v-if="healthProductsDialog.status == 'update'" type="primary" @click="changeHealthProducts">确认</el-button>
	  	</div>
		</el-dialog>
	</div>
...
<script>
import { getAllHealthProducts, healthProducts, updateHealthProducts } from '@/api/healthProducts'
...
export default {
  ...
  methods: {
    ...
    getAllHealthProducts() {
      ...
      getAllHealthProducts().then(response => {
        this.healthProducts = response.data
        this.listLoading = false
      })
    },
    updateHealthProducts(row) {
      this.temp = Object.assign({}, row)
      this.healthProductsDialog.visiable = true
      this.healthProductsDialog.title = '修改酒类信息'
      this.healthProductsDialog.status = 'update'
    },
    changeHealthProducts() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          updateHealthProducts(this.temp).then(res => {
            if (res.code === 200) {
              this.getAllHealthProducts()
              this.healthProductsDialog.visiable = false
              this.$message({
                type: 'success',
                message: '修改成功!'
              })
      ...
      this.$refs['rfidForm'].validate((valid) => {
        if (valid) {
          if (this.rfidDialog.status === 'update') {
            ...
            this.commoditiesCode.commoditiesType = 3
            addCommoditiesCode(this.commoditiesCode).then(res => {
              if (res.code === 200) {
                if (res.data === '105') {
                  ...
                  this.temp = this.healthProducts[0]
                  updateHealthProducts(this.temp).then(res => {
                    if (res.code === 200) {
                      this.getAllHealthProducts()
                    }
...
<style>
  #healthProducts .el-dialog__body {
    padding: 0 0 0 0;
    width: 100%;
    text-align: center;
  }
  #healthProducts .el-form-item {
    width: 40%;
    display: inline-block;
    margin: 0 0 15px 0;
  }
  #healthProducts .el-form-item__label {
    float: left;
    width: 29%;
  }
  #healthProducts .el-form-item__content{
    display: inline-block;
  }
  #healthProducts #pigarea {
    width: 100%;
  }
  #healthProducts #pigarea .el-form-item__label {
    display: block;
    width: 100%;
    text-align: center;
  }
  #healthProducts #pigarea .el-form-item__content {
    display: block;
    width: 50%;
    margin: 0 0 0 25%;
  }
  #healthProducts #pigarea .el-form-item__content textarea{
    text-align: center;
    height: 100px;
  }
  #healthProducts .el-table--mini td {
    padding: 0;
    height: 35px;
    font-size: 12px;
  }
  #healthProducts .el-table--mini th {
    padding: 0;
    height: 40px;
    font-size: 14px;
  }
